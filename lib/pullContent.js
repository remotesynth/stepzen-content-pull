...
exports.runQueries = function (config, apiKey) {
  ...
  if (!config.account_name || config.account_name === '')
    throw new Error('Missing config value for hostname');

  if (!config.endpoint || config.endpoint === '')
    throw new Error('Missing config value for path');
  ...
  config.queries.map((query) => {
    let req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        let theData = results.data[keys[0]];
        if (query.root_node && query.root_node != '')
          theData = flattenResults(theData, query.root_node.split('.'));
        ...
        if (fileType === 'json') {
          ...
          throw new Error(
            'Unsupported file type. Supported file types: json, md, markdown and yaml'
          );
        }
      });
    });
...
//removes double slashes from the destination caused by the the destination being an empty string (i.e. root folder)
function fixDestination(destination) {
  const slashesRegex = /\/\//g;
  return destination.replace(slashesRegex, '/');
}
// removes nested nodes based upon the root specified in the config
function flattenResults(theData, nodesArr) {
  let newNodes = [...nodesArr];
  nodesArr.every((node) => {
    if (node.includes('[]')) {
      theData = theData[node.replace('[]', '')];
      newNodes.shift();
      let arrData = theData.map((item) => {
        return flattenResults(item, newNodes);
      });
      theData = arrData;
      return false;
    } else {
      theData = theData[node];
      newNodes.shift();
      return true;
    }
  });
